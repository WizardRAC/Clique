...
from CliqueAI.graph.random_generator import generate_random_graph_with_planted_clique
...
from common.base.wandb_logging.mock_client import MockWandbClient
...
class Validator(BaseValidatorNeuron):
    ...
    def __init__(self, config=None):
        ...
        use_random_graph = getattr(self.config, "use_random_graph", False)
        
        if use_random_graph:
            # Disable wandb completely when using random graphs for testing
            self.wandb_client = None
            bt.logging.info("WandB logging disabled for testing mode")
        elif not self.config.wandb.off:
            self.wandb_client = MockWandbClient(
                wallet=self.wallet,
                base_version=str(validator_int_version),
    ...
    async def forward(self):
        ...
        if use_random_graph:
            bt.logging.info("Using random graph generation for testing")
            # Calculate clique size based on problem difficulty
            if problem.difficulty == 0.1:  # Easy
                clique_size = random.randint(8, 12)
            elif problem.difficulty == 0.2:  # Medium
                clique_size = random.randint(15, 20)
            else:  # Hard (0.4)
                clique_size = random.randint(20, 30)
            
            graph = generate_random_graph_with_planted_clique(
                label=problem.label,
            ...
            bt.logging.info(f"Generated random graph with planted clique of size {clique_size}")
        else:
            # Use Lambda service (production)
            try:
        ...
        if self.wandb_client is not None and not self.config.wandb.off and len(selected_uids) > 0:
